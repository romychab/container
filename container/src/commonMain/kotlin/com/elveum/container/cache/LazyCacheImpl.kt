...
internal class LazyCacheImpl<Arg, T>(
    ...
    ): LazyCache<Arg, T> = lock.withLock {
        whenActiveBlocks.add(block)
        this
    }

    private fun registerRecord(arg: Arg): CacheRecord<T> = lock.withLock {
    ...
    private fun unregisterRecord(arg: Arg) = lock.withLock {
        val record = cacheSlots[arg] ?: return@withLock
        record.count--
        if (record.count == 0) {
            scheduleRemoval(arg)
        }
    ...
    private fun getSubject(arg: Arg): LazyFlowSubject<T>? = lock.withLock {
