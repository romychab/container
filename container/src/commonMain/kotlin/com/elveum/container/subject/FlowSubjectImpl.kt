package com.elveum.container.subject

import com.elveum.container.internal.Lock
import com.elveum.container.internal.withLock
import kotlinx.coroutines.channels.Channel
import kotlinx.coroutines.channels.awaitClose
import kotlinx.coroutines.flow.Flow
import kotlinx.coroutines.flow.buffer
import kotlinx.coroutines.flow.callbackFlow

internal class FlowSubjectImpl<T> : FlowSubject<T> {

    private val lock = Lock()
    private var state: State<T> = State.NotInitialized
    private val listeners = mutableSetOf<Listener<T>>()

    override fun onNext(value: T) = lock.withLock {
        if (state is State.Closed) return@withLock
        state = State.WithValue(value)
        listeners.forEach { it.onNext(value) }
    }

    override fun onError(e: Exception) = lock.withLock {
        if (state is State.Closed) return@withLock
        state = State.Closed(e, null)
        listeners.forEach { it.onComplete(e) }
    }

    override fun onComplete() = lock.withLock {
        val state = this.state
        if (state is State.Closed) return@withLock
        val value = if (state is State.WithValue) {
            state.value
        } else {
            null
        }
        this.state = State.Closed(null, value)
        listeners.forEach {
            it.onComplete()
        }
    }

    override fun flow(): Flow<T> {
        return callbackFlow {
            val listener = object : Listener<T> {
                override fun onNext(value: T) {
                    trySend(value)
                }

                override fun onComplete(e: Exception?) {
                    channel.close(e)
                }
            }
            add(listener)
            awaitClose {
                remove(listener)
            }
        }.buffer(Channel.UNLIMITED)
    }

    private fun add(listener: Listener<T>) = lock.withLock {
        val state = this.state
        if (state is State.Closed) {
            if (state.value != null) listener.onNext(state.value)
            listener.onComplete(state.exception)
        } else {
            listeners.add(listener)
            if (state is State.WithValue) {
                listener.onNext(state.value)
            }
        }
    }

    private fun remove(listener: Listener<T>) = lock.withLock {
        listeners.remove(listener)
    }

    private interface Listener<T> {
        fun onNext(value: T)
        fun onComplete(e: Exception? = null)
    }

    private sealed class State<out T> {
        data object NotInitialized : State<Nothing>()
        class WithValue<T>(val value: T) : State<T>()
        class Closed<T>(val exception: Exception?, val value: T?): State<T>()
    }
}
