...
public interface LazyFlowSubject<T> {
    ...
    public companion object {
        ...
        public fun <T> create(
            ...
            // This single Lock must be threaded into LoadTaskManager, LazyFlowSubjectImpl and
            // (via its default) FlowDependencyStoreImpl: they used to synchronize on one shared
            // monitor before the migration, and separate Lock() defaults would silently split it.
            val lock = Lock()
            return LazyFlowSubjectImpl(
                coroutineScopeFactory = coroutineScopeFactory,
                cacheTimeoutMillis = cacheTimeoutMillis,
                loadTaskManager = LoadTaskManager(transformation, loaderDecorator, lock),
