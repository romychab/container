...
import com.elveum.container.platform.PlatformMonitor
...
internal class PageLoaderRecordStore<Key, T>(
    ...
    internal val monitor = PlatformMonitor()
    ...
    fun forEach(action: (PageKeyRecord<Key, T>) -> Unit) = monitor.synchronized {
        orderedRecords.values.forEach(action)
    }

    fun findNextKeyForIndex(index: Int): NextKeyLoadResult<Key> = monitor.synchronized {
    ...
    fun getOrPut(key: Key, defaultValue: () -> PageKeyRecord<Key, T>): PageKeyRecord<Key, T>? = monitor.synchronized {
    ...
    fun updateContainer(key: Key, container: Container<List<T>>) = monitor.synchronized {
    ...
    fun hasLaunchedKey(key: Key) = monitor.synchronized {
        orderedRecords[key]?.job != null
    }

    fun onKeyContinued(key: Key) = monitor.synchronized {
        orderedRecords[key]?.container = pendingContainer()
    }

    fun onKeyCompleted(key: Key) = monitor.synchronized {
        counter--
    ...
    fun shutdown() = monitor.synchronized {
        counter = 0
        isFinished = true
    ...
    fun onKeyFailed(key: Key, exception: Exception) = monitor.synchronized {
        val nonFinalOldItems = getNonFinalItems()
    ...
    fun buildOutputList(): List<T> = monitor.synchronized {
        return outputList.toList()
    }

    fun getAllContainers() = monitor.synchronized {
        orderedRecords.values.map { it.container }
    }

    fun firstOrNull(
        predicate: (PageKeyRecord<Key, T>) -> Boolean,
    ): PageKeyRecord<Key, T>? = monitor.synchronized {
    ...
    fun keys(): List<Key> = monitor.synchronized {
        orderedRecords.keys.toList()
    }

    fun intercept(container: Container<List<T>>): Container<List<T>> = monitor.synchronized {
