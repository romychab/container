...
internal abstract class AbstractLazyFlowSubjectIntegrationTest {
    ...
    protected fun FlowTestScope.createLazyFlowSubject(
        ...
        // Mirror production's LazyFlowSubject.create(): LoadTaskManager, LazyFlowSubjectImpl
        // and FlowDependencyStoreImpl must share one Lock instance.
        val lock = Lock()
        return LazyFlowSubjectImpl<String>(
            coroutineScopeFactory = coroutineScopeFactory,
            cacheTimeoutMillis = cacheTimeout,
            loadTaskManager = LoadTaskManager(loaderDecorator = loaderDecorator, lock = lock),
            lock = lock,
        ).apply {
            if (loader != null) {
