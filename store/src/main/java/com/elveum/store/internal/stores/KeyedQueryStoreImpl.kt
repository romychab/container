...
internal class KeyedQueryStoreImpl<Key : Any, Q : Any, T : Any>(
    ...
    override suspend fun invalidate(key: Key, metadata: ContainerMetadata) =
        coreStore.invalidate(key, metadata)

    override fun invalidateAsync(key: Key, metadata: ContainerMetadata) =
        coreStore.invalidateAsync(key, metadata)
